<<<<<<< HEAD:admin/dist/assets/D3WordCloud-6ohoyviu4u.js
<<<<<<< HEAD:admin/dist/assets/D3WordCloud-myg4akepfo.js
...
=======
import { l as getAugmentedNamespace, r as reactExports, a as useQueryClient, h as useQuery, R as React, e as getFetch, m as getParamsChar } from "../main-6ohoyviu4u.js";
import { S as Selection, r as root, R as Rgb, a as rgbConvert, d as define, e as extend, C as Color, b as brighter, c as darker, f as constant, i as interpolateNumber, g as color, h as interpolateRgb, j as interpolateString, n as nogamma, k as hue } from "./transform-6ohoyviu4u.js";
import "./OverviewTemplate-6ohoyviu4u.js";
>>>>>>> 18bd270e (custom vite build & watch):admin/dist/assets/D3WordCloud-6ohoyviu4u.js
=======
import { l as getAugmentedNamespace, r as reactExports, a as useQueryClient, h as useQuery, R as React, e as getFetch, m as getParamsChar } from "../main-wnppnrkdix.js";
import { S as Selection, r as root, R as Rgb, a as rgbConvert, d as define, e as extend, C as Color, b as brighter, c as darker, f as constant, i as interpolateNumber, g as color, h as interpolateRgb, j as interpolateString, n as nogamma, k as hue } from "./transform-wnppnrkdix.js";
import "./OverviewTemplate-wnppnrkdix.js";
>>>>>>> 63c8f541 (feat: add popup content):admin/dist/assets/D3WordCloud-wnppnrkdix.js
...
function cloudSprite(contextAndRatio, d, data, di) {
  ...
  while (++di < n) {
    ...
    if (y + h >= ch)
      break;
    c2.translate((x + (w >> 1)) / ratio, (y + (h >> 1)) / ratio);
    if (d.rotate)
...
const D3WordCloud = ({ slug }) => {
  ...
  const { data } = useQuery({
    ...
    queryFn: () => {
      const response = getFetch(`${slug}` + getParamsChar() + `filter_kw_usage_count=%7B%22op%22%3A%22%3E%22%2C%22val%22%3A0%7D${filters !== void 0 ? filters : ""}&sort_column=kw_usage_count&sort_direction=DESC&rows_per_page=1000`).then((chartData) => {
        return chartData;
      });
      if (response.ok) {
  ...
  reactExports.useEffect(() => {
    ...
    const minCount = min(data, (d) => d.kw_usage_count);
    const maxCount = max(data, (d) => d.kw_usage_count);
    const fontSizeScale = linear().domain([minCount, maxCount]).range([10, 150]);
    const colorScale = sequential(interpolateRainbow).domain([0, data.length]);
    const layout = d3Cloud().size([900, 700]).words(data.map((d) => ({ text: d.keyword, kw_usage_count: d.kw_usage_count }))).padding(5).rotate(() => ~~(Math.random() * 2) * 90).font("Impact").fontSize((d) => fontSizeScale(d.kw_usage_count)).on("end", draw);
    layout.start();
    function draw(words) {
      select(chartRef.current).append("svg").attr("width", layout.size()[0]).attr("height", layout.size()[1]).append("g").attr("transform", `translate(${layout.size()[0] / 2}, ${layout.size()[1] / 2})`).selectAll("text").data(words).enter().append("text").style("font-size", (d) => fontSizeScale(d.kw_usage_count)).style("font-family", "Impact").style("fill", (d, i) => colorScale(i)).attr("text-anchor", "middle").attr("transform", (d) => "translate(" + [d.x, d.y] + ")rotate(" + d.rotate + ")").text((d) => d.text);
    }
  }, [data]);
  return /* @__PURE__ */ React.createElement("div", { className: "urlslab-overview urlslab-panel fadeInto" }, /* @__PURE__ */ React.createElement("div", { ref: chartRef }));
};
export {
  D3WordCloud as default
};
