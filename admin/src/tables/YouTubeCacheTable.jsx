...
function YouTubeCacheTable( { slug } ) {
	...
	const setUnifiedPanel = useCallback( ( cell ) => {
		...
		if ( origCell.usage_count > 0 ) {
			setOptions( [ {
				detailsOptions: {
					title: `${ __( 'Video ID', 'wp-urlslab' ) } “${ origCell.videoid }” ${ __( 'is used on these URLs', 'wp-urlslab' ) }`, text: `${ __( 'Video title:', 'wp-urlslab' ) } ${ cell.row._valuesCache.title[ 1 ] }`, slug, url: `${ origCell.videoid }/urls`, showKeys: [ { name: [ 'url_name', __( 'URL', 'wp-urlslab' ) ] } ], listId: 'url_id', counter,
				},
			} ] );
		}
