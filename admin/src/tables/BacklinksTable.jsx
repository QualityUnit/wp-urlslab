...
const header = {
	...
	from_http_status: __( 'From URL Status' ),
	to_url_name: __( 'To URL' ),
	anchor_text: __( 'Anchor text' ),
	status: __( 'Anchor text' ),
	created: __( 'Created' ),
	updated: __( 'Updated' ),
	...
	const columns = useMemo( () => [
		...
		columnHelper?.accessor( 'from_http_status', {
			...
			cell: ( cell ) => (
				<Stack direction="row" alignItems="center" spacing={ 1 }>
					<>
						<ActionHTTPStatusButton cell={ cell } onClick={ ( val ) => updateRow( { changeField: 'from_http_status', newVal: val, cell } ) } />
		...
		columnHelper.accessor( 'editRow', {
			...
			cell: ( cell ) => <RowActionButtons
				onEdit={ () => updateRow( { cell, id: 'from_url_name' } ) }
				onDelete={ () => deleteRow( { cell, id: 'from_url_name' } ) }
			>
			</RowActionButtons>,
			header: () => null,
			size: 0,
	...
	return (
		<>
			...
			<Table className="fadeInto"
				initialState={ { columnVisibility: { created: false, updated: false, note: false, anchor_text: false } } }
				columns={ columns }
...
const TableEditorManager = memo( ( { slug } ) => {
	...
	const rowEditorCells = useMemo( () => ( {
		from_url_name: <InputField liveUpdate autoFocus defaultValue="" label={ header.from_url_name } onChange={ ( val ) => {
			setRowToEdit( { from_url_name: val } );
		} } required description={ __( 'Enter URL of page where you arranged backlink to your website and you want to monitor it.' ) } />,

		to_url_name: <InputField liveUpdate defaultValue="" label={ header.to_url_name } onChange={ ( val ) => {
			setRowToEdit( { to_url_name: val } );
		} } required description={ __( 'Specify the URL included in the content of the monitored webpage.' ) } />,

		note: <TextArea rows="5" description={ __( 'Add a note about the monitored backlink. For example, record the contact details of the partner responsible for managing the backlink.' ) }
						liveUpdate defaultValue="" label={ header.note } onChange={ ( val ) => setRowToEdit( { note: val } ) } />,

		labels: <TagsMenu optionItem label={ header.labels } slug={ slug } onChange={ ( val ) => setRowToEdit( { labels: val } ) } />,
