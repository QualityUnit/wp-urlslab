...
const title = __( 'Add Query', 'wp-urlslab' );
	...
	const ActionButton = useMemo( () => ( { cell, onClick } ) => {
		...
		return (
			<div className="flex flex-align-center flex-justify-end">
				...
				{ ( serpStatus !== 'P' ) &&
					<Tooltip title={ __( 'Process again', 'wp-urlslab' ) } arrow placement="bottom">
...
export const getQueriesTableEditorCells = ( { data, onChange, tableSlug, columnTypes } ) => {
	return {
		query: <TextArea autoFocus liveUpdate defaultValue={ data.query ? data.query : '' } label={ __( 'Queries', 'wp-urlslab' ) } rows={ 10 } allowResize onChange={ ( val ) => onChange( { query: val } ) } required description={ __( 'Each query must be on a separate line', 'wp-urlslab' ) } />,
		country: <CountrySelect label={ queryHeaders.country } value={ data.country ? data.country : 'us' } onChange={ ( val ) => onChange( { country: val } ) } />,
		schedule_interval: <SingleSelectMenu liveUpdate autoClose defaultAccept defaultValue={ data.schedule_interval ? data.schedule_interval : '' } description={ __( 'Select how often should be SERP data updated. Each query update costs small fee. System defauld value can be changed in Settings of SERP module.', 'wp-urlslab' ) } onChange={ ( val ) => onChange( { schedule_interval: val } ) } items={ columnTypes?.schedule_interval.values }>{ queryHeaders.schedule_interval }</SingleSelectMenu>,
		labels: <TagsMenu optionItem defaultValue={ data.labels ? data.labels : '' } label={ __( 'Tags:', 'wp-urlslab' ) } slug={ tableSlug } onChange={ ( val ) => onChange( { labels: val } ) } />,
	};
};
