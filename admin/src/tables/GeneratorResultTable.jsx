...
function GeneratorResultTable( { slug } ) {
	...
	const setUnifiedPanel = useCallback( ( cell ) => {
		...
		if ( origCell.usage_count > 0 ) {
			setOptions( [ {
				detailsOptions: {
					title: __( 'Shortcode used on these URLs', 'wp-urlslab' ), slug, counter, url: `${ origCell.shortcode_id }/${ origCell.hash_id }/urls`, showKeys: [ { name: [ 'url_name', __( 'URL', 'wp-urlslab' ) ] }, { name: [ 'created', __( 'Created', 'wp-urlslab' ) ] } ], perPage: 999, listId: 'url_id',
				},
			} ] );
		}
