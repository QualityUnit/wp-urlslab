...
export const preprocessUrls = async ( data, processing = 0, signal ) => {
	try {
		setNotification( 'serp-gap/prepare/download-failed', {
			title: __( 'Downloading URLs…', 'wp-urlslab' ),
			message: __( 'It can take few minutes.', 'wp-urlslab' ),
			status: 'info',
		} );
		...
		throw new Error( __( 'Failed to process URLs.', 'wp-urlslab' ) );
	} catch ( error ) {
		if ( error !== 'cancelled' ) {
...
const validateResults = ( results ) => {
	...
	if ( withError.length ) {
		setNotification( 'serp-gap/prepare/download-failed', {
			title: __( 'Download of some URLs failed.', 'wp-urlslab' ),
			message: __( 'Some data will not be present in table.', 'wp-urlslab' ),
			status: 'error',
		} );
	}
	return results;
