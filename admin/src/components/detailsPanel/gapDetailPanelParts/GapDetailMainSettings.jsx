		...
		<SettingsWrapper>
			...
			<Box>
				<Stack spacing={ 1 }>
					<FormControl>
						<FormLabel flexNoWrap textNoWrap>
							{ __( 'Parse text from', 'wp-urlslab' ) }
							<Tooltip
								title={
									<>
										<strong>{ __( 'How parsing works?', 'wp-urlslab' ) }</strong>
										<p>{ __( 'Text elements from specified URLs will be extracted and compared for phrase matching. Checking this box allows for parsing text strictly from headers, i.e. H1 … H5 tags, and TITLE tags. This is a useful option as copywriters often use the most important keywords in titles and headers, thus enabling the identification of keyword frequency based on headings alone.', 'wp-urlslab' ) }</p>
									</>
								}
								placement="bottom"
								sx={ { maxWidth: '45rem' } }
					...
					<FormControl>
						<FormLabel flexNoWrap textNoWrap>
							{ __( 'Word combinations', 'wp-urlslab' ) }
							<Tooltip
								title={ __( 'The "n-grams" field specifies the maximum number of words that are grouped together to form a compound keyword for search queries. For instance: 1-gram (unigram) consists of a single word (e.g., "apple"), a 2-gram (bigram) incorporates a pair of words (e.g., "fresh apple"), etc.', 'wp-urlslab' ) }
								placement="bottom"
								sx={ { maxWidth: '45rem' } }
							>
								<Box>
					...
					<FormControl orientation="horizontal">
						<MuiCheckbox
							...
							label={ __( 'Show just queries from Keyword Cluster', 'wp-urlslab' ) }
						...
						<Tooltip
							title={
								<>
									<strong>{ __( 'What is keyword cluster?', 'wp-urlslab' ) }</strong>
									<p>{ __( 'Cluster forms keywords discovered in your database, where the same URLs rank on Google Search for each query.', 'wp-urlslab' ) }</p>
									<p>{ __( 'Based on entered query we identify all other best matching keywords from the cluster.', 'wp-urlslab' ) }</p>
									<p>{ __( 'If this option is selected, keywords included in page, but not found in SERP data will not be included in the table.', 'wp-urlslab' ) }</p>
								</>
							}
							placement="bottom"
							sx={ { maxWidth: '45rem' } }
					...
					<FormControl orientation="horizontal" >
						<MuiCheckbox
							...
							label={ __( 'Compare domains of URLs', 'wp-urlslab' ) }
							sx={ ( theme ) => ( { color: theme.palette.urlslabColors.greyDarker } ) }
							textNoWrap
						/>
						<Tooltip
					...
					<Stack direction="row" spacing={ 1 } >
						<FormControl sx={ { marginBottom: 1 } }>
							<FormLabel>{ __( 'Clustering level', 'wp-urlslab' ) }</FormLabel>
							<Input
								type="number"
						...
						<FormControl sx={ { marginBottom: 1 } }>
							<FormLabel>{ __( 'Max position', 'wp-urlslab' ) }</FormLabel>
							<Input
								type="number"
