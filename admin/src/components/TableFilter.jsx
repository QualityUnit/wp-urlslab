	...
	return (
		<div className="flex flex-align-center flex-wrap">
					...
					! ( hiddenFilters && hiddenFilters.includes( keyWithoutId ) )
						...
						>
							...
							&nbsp;
									...
									{ keyWithoutId === 'labels'
										...
										`min: ${ filterValue.min }, max: ${ filterValue.max }`
											...
											{ keyWithoutId === 'browser' &&
												( ( filters[ key ]?.val.browser ? `${ browsers[ filters[ key ]?.val.browser[ 0 ] ] || filters[ key ]?.val.browser[ 0 ] } ${ filters[ key ]?.val.system ? __( 'on', 'wp-urlslab' ) + ' ' + filters[ key ]?.val.system : '' }` : filters[ key ]?.val.system ) || ' ' + __( 'bot', 'wp-urlslab' ) + ' ' + filters[ key ]?.val.bot )
											}
								...
								<Tooltip className="showOnHover">{ __( 'Edit filter', 'wp-urlslab' ) }</Tooltip>
							</div>
							<div className="flex flex-align-center">
								...
								<Tooltip className="showOnHover" style={ { width: '8em' } }>{ __( 'Delete filter', 'wp-urlslab' ) }</Tooltip>
							</div>
			...
			{ activefilters?.length > 0 && // Removes all used filters in given table
				...
				} }>{ __( 'Clear filters', 'wp-urlslab' ) }</Button>
			}
		</div>
	);
}
