		...
		<>
			{ tooltipLabel
				? <div className="urlslab-tooltipLabel flex flex-align-center">
					<span
						className={ `urlslab-inputField-label ${ required ? 'required' : '' }` }>{ tooltipLabel.label }</span>
					<InfoTooltipIcon text={ tooltipLabel.tooltip } noWrapText />
				</div>
				: null
			}
			<JSXSingleSelectMenu className={ className } name={ name } style={ style } items={ items }
				description={ description } defaultValue={ defaultValue }
				defaultAccept={ defaultAccept } autoClose={ autoClose } disabled={ disabled }
				isFilter={ isFilter } required={ required }
				onChange={ onChange }>{ tooltipLabel ? null : children }</JSXSingleSelectMenu>
		</>
	);
} );
...
export const Tooltip: React.FC<TooltipType> = React.memo( ( {
	...
	return <JSXTooltip active={ active } center={ center } className={ className }
		style={ { ...style, ...( width && { width } ) } }>{ children }</JSXTooltip>;
} );
...
export const Checkbox: React.FC<CheckboxType> = React.memo( ( {
	...
	return <JSXCheckbox defaultValue={ defaultValue } smallText={ smallText } readOnly={ readOnly } radial={ radial }
		name={ name } className={ className } onChange={ onChange }
		textBefore={ textBefore }>{ children }</JSXCheckbox>;
} );
...
export const Button: React.FC<ButtonType> = React.memo( ( {
	...
	return <JSXButton active={ active } danger={ danger } type={ type } className={ className } disabled={ disabled }
		onClick={ onClick } href={ href } target={ target }>{ children }</JSXButton>;
} );
...
export const TextArea: React.FC<TextAreaType> = React.memo( ( {
	...
	return <JSXTextArea defaultValue={ defaultValue } autoFocus={ autoFocus } placeholder={ placeholder }
		liveUpdate={ liveUpdate } className={ classes } readonly={ readonly } disabled={ disabled }
		label={ label } description={ description } labelInline={ labelInline } required={ required }
		onChange={ onChange } style={ style } rows={ rows }>{ children }</JSXTextArea>;
} );
