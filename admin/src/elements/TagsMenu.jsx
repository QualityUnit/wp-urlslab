...
const TagsPopupContent = memo( () => {
	...
	return (
		<Stack spacing={ 1.5 } >
			...
			<Box>
				<Typography level="body-xs" sx={ { textTransform: 'uppercase', fontWeight: 600, mb: 1 } }>{ __( 'Add tags:', 'urlslab' ) }</Typography>
				{ maxTagsReached
					// translators: %i is number of maximum allowed tags, do not change it.
					? <Alert size="sm" variant="soft" color="danger">{ __( 'Maximum of %i tags are allowed.', 'urlslab' ).replace( '%i', maxTags ) }</Alert>
					: <>
						<Input
							value={ searchText }
							size="sm"
							color="neutral"
							variant="outlined"
							placeholder={ __( 'Filter available tags…', 'urlslab' ) }
							...
							>
								{
									...
									__( 'Create new tag %s', 'urlslab' ).replace( '%s', `"${ searchText }"` )
								}
							</Button>
						</>
						}
	...
	return (
		<Tooltip
			...
			// translators: %i is integer of maximum allowed tags, do not change it.
				__( 'Maximum of %i tags are allowed.', 'urlslab' ).replace( '%i', maxTags )
			}
		>
