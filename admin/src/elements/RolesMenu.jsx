...
export default function RolesMenu( { defaultValue, noLabel, disabled, description, onChange } ) {
	...
	return <SingleSelectMenu disabled={ disabled } description={ description } items={ roleNames } name="roles" autoClose defaultValue={ ( ! disabled && defaultValue ) || 'none' } onChange={ ( val ) => onChange( val === 'none' ? '' : val ) }>{ ! noLabel && __( 'Roles' ) }</SingleSelectMenu>;
}
