...
export async function handleApiError( notificationId, response, options ) {
	...
	try {
		...
		//json parsing failed
		setNotification( notificationId, { title, message: message ? message : __( 'API request failed.', 'wp-urlslab' ), status: 'error' } );
	...
	setNotification( notificationId, { title, message: __( 'API request failed.', 'wp-urlslab' ), status: 'error' } );
}
