		...
		<Overview moduleId={ moduleId } section={ ( val ) => setSection( val ) } noIntegrate>
			{
				section === 'about' &&
				<section>
					<p>{ __( 'Managing a website with a large content volume can be a hassle. Luckily, URLsLab’s Search and Replace module can save you time and frustration by automatically mass-replacing URLs and content on the fly.' ) }</p>
			...
			}
			{
				section === 'faq' &&
					<section>
						<h4>{ __( 'How does the Search and Replace module handle incorrect URLs?' ) }</h4>
						<p>{ __( 'Incorrect or outdated URLs are in the Search and Replace module’s realm of capabilities. You can replace incorrect URLs in bulk, or one one by. This is especially useful when rebranding your website.' ) }</p>
						<h4>{ __( 'Can I undo changes made by the Search and Replace module?' ) }</h4>
						<p>{ __( 'Yes, you can. With a single click, you can revert any changes. This way you don’t have to worry about making permanent changes in the database and not having a reliable fallback method.' ) }</p>
					</section>
			}
		</Overview>
	);
