	...
	return (
		<Overview moduleId={ moduleId } section={ ( val ) => setSection( val ) } noIntegrate>
			...
			}
			{
				section === 'faq' &&
					<section>
						<h4>{ __( 'How many FAQs can I add to each post?' ) }</h4>
						<p>{ __( 'By default, the Frequently Asked Questions module generates 8 questions and answers, however, you can change this number in the Settings tab.' ) }</p>
						<h4>{ __( 'Will the FAQs be relevant to the content of my posts?' ) }</h4>
						<p>{ __( 'Yes, the automatically generated FAQs will be based on your post, and therefore, they will be contextually relevant. On top of that, you can create custom FAQs for each post.' ) }</p>
						<h4>{ __( 'How does the Frequently Asked Questions module enhance user experience on my website?' ) }</h4>
						<p>{ __( 'In the online space, convenience is everything. Adding FAQs to your posts allows users to find information quickly and easily. Furthermore, the more user-oriented elements you add to your website, the more engaged your visitors will be.' ) }</p>
					</section>
			}
		</Overview>
	);
