		...
		<Overview moduleId={ moduleId } section={ ( val ) => setSection( val ) }>
			{
				section === 'about' &&
				<section>
					<p>{ __( 'Lazy loading is an essential technique for optimizing page performance and reducing page load time. Traditionally, all the images and videos are loaded upon accessing a website. By using lazy loading, this content is loaded only when it enters a user’s viewport.' ) }</p>
			...
			}
			{
				section === 'integrate' &&
					<section>
						<h4>{ __( 'Video data shortcode' ) }</h4>
						<code>[urlslab-video videoid="YT-video-ID" attribute="atr_name"]</code>
						<p>{ __( 'Supported video attribute names: title, description, thumbnail_url, published_at, duration, captions, captions_text, channel_title' ) }</p>
					</section>
			}
			{
				section === 'faq' &&
					<section>
						<h4>{ __( 'What are the SEO benefits of using the Lazy Loading module?' ) }</h4>
						<p>{ __( 'Websites with peak performance are usually favored by search engines. When you use lazy loading, you can decrease page loading times, and improve your overall website performance.' ) }</p>
						<h4>{ __( 'Does the Lazy Loading allow me to specify which parts of my web page to lazy load?' ) }</h4>
						<p>{ __( 'Using URLsLab’s Lazy Loading module, you can select which web page parts to lazy load. For controlling content lazy loading, extra parameters can be defined in the content settings.' ) }</p>
					</section>
			}
		</Overview>
	);
