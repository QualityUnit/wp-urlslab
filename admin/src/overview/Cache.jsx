		...
		<Overview moduleId={ moduleId } section={ ( val ) => setSection( val ) } noIntegrate>
			{
				section === 'about' &&
				<section>
					<p>{ __( 'Search engines prefer websites that load quickly and perform well. Keeping your website performance up is a necessary, although often difficult task. Our state-of-the-art caching module is engineered to boost your website’s loading speed and overall performance.' ) }</p>
					<h5>{ __( 'Cache rules' ) }</h5>
					...
					<h5>{ __( 'CloudFront Support' ) }</h5>
					<p>{ __( 'The plugin includes a connector specifically designed for website owners using WordPress installations behind AWS CloudFront CDN. This feature allows you to conveniently purge the cache of a specific page or the entire website as required.' ) }</p>
				</section>
			}
			{
				section === 'faq' &&
				<section>
					<h4>{ __( 'Can I specify a cache validity period?' ) }</h4>
					<p>{ __( 'Yes, you can. In the Settings section, you can set up the lifespan of cached objects, as well as newly added items. This gives you complete control over the Cache module’s behavior. There is one global setting for page cache validity and optionally you can rewrite default value with custom rules.' ) }</p>
					<h4>{ __( 'How can the Cache module help my business save money?' ) }</h4>
					<p>{ __( 'By conserving server load and bandwidth, the Cache module can help your business save valuable resources. By delivering cached versions of your pages, the module eliminates the need for retrieving redundant data, which leads to reduced server load and bandwidth consumption.' ) }</p>
					<h4>{ __( 'Can the Cache module handle high-traffic websites?' ) }</h4>
					...
					<h4>{ __( 'Does the Cache module require regular maintenance or updates?' ) }</h4>
					<p>{ __( 'No, it does not. All you need to do is set up caching rules and the module is good to go. It is also possible to perform a manual action, such as invalidating the cache immediately.' ) }</p>
				</section>
			}
		</Overview>
	);
