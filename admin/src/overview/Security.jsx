		...
		<Overview moduleId={ moduleId } section={ ( val ) => setSection( val ) } noIntegrate>
			{
				section === 'about' &&
				<section>
					<h2>{ __( 'Protect your website against simple attacks' ) }</h2>
					<p>{ __( 'Security is a crucial aspect of any website. The Security module helps you protect your website against simple attacks, such as SQL injections and XSS attacks. With URLsLab you can set CSP headers, block attackers visiting random 404 Not Found pages or protect against simple DDoS attacks from single IP address.' ) }</p>
					<h2>{ __( 'Threats' ) }</h2>
					<h3>{ __( 'DDoS attacks' ) }</h3>
					...
					<h3>{ __( 'Mitigating cross-site scripting' ) }</h3>
					...
					<h3>{ __( 'Mitigating packet sniffing attacks' ) }</h3>
