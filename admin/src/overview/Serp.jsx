		...
		<Overview moduleId={ moduleId } section={ ( val ) => setSection( val ) } noIntegrate>
			{
				section === 'about' &&
				<section>
					<p>{ __( 'Many websites struggle with maintaining or improving their visibility online. Luckily, URLsLab’s SEO Insights tool is here to help. It improves your online visibility by identifying relevant search terms for your industry, organizing them for content creation, and providing competitor insights.' ) }</p>
					...
					<p>{ __( 'Implementing the SEO Insights module can lead to more data-driven decision-making processes due to its dashboards filled with valuable information. Additionally, employing this module gives you a competitive advantage, improves your online visibility, and helps attract wider audiences.' ) }</p>
				</section>
			}
			{
				section === 'faq' &&
					<section>
						<h4>{ __( 'Does the SERP data analysis cost extra credits?' ) }</h4>
						<p>{ __( 'Yes, it does. Although fetching data from the Google Search Console doesn’t cost extra, your credit will decrease for every tracked query in SERP.' ) }</p>
						<h4>{ __( 'How does the SEO Insights module help in improving my website’s ranking?' ) }</h4>
