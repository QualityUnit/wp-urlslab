		...
		<Overview moduleId={ moduleId } section={ ( val ) => setSection( val ) } noIntegrate>
			{
				...
				<section>
					...
					<p>{ __( 'Through the Link Building module, you can monitor and manage all the keywords throughout your entire website in a convenient and easy-to-navigate interface. In addition to extracting all the keywords from your pages, the Keyword list contains vital information such as the linked URL, language, SEO rank, and usage.' ) }</p>
					...
					<p>{ __( 'Using the Link Building module to strengthen the internal links structure on your website comes with numerous benefits, including enhanced SEO rankings, improved user experience, and overall engagement.' ) }</p>
					<h3>{ __( 'Backlink monitoring' ) }</h3>
			...
			}
			{
				section === 'faq' &&
					<section>
						<h4>{ __( 'Can I customize the Link Building module?' ) }</h4>
						<p>{ __( 'Yes, you can set up the Link Building module to suit your needs and requirements. You can customize the module’s functionalities in the Settings tab.' ) }</p>
						<h4>{ __( 'Can I import keywords using the Link Building module?' ) }</h4>
						<p>{ __( 'Yes, you can. The Link Building module allows you to import both internal and external keywords.' ) }</p>
						<h4>{ __( 'How does the Link Building module help improve my website’s SEO ranking?' ) }</h4>
						<p>{ __( 'By improving your internal links structure, the Link Building module contributes to the overall SEO ranking of your website. Implementing this tool can lead to higher SERP scores, improved organic traffic, and visitor engagement.' ) }</p>
					</section>
			}
		</Overview>
	);
