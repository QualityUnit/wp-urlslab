...
class Urlslab_Widget_Web_Vitals extends Urlslab_Widget {
	...
	protected function add_options() {
		...
		$this->add_option_definition(
			...
			false,
			true,
			__( 'Activate Web Vitals Measurement', 'urlslab' ),
			__( 'The plugin will include a small JavaScript library in the website\'s source code. This library measures web vitals in the browser of your visitors as they browse your page.', 'urlslab' ),
		...
		$this->add_option_definition(
			...
			true,
			true,
			__( 'CLS Measurements', 'urlslab' ),
			__( 'Enable logging for Cumulative Layout Shift (CLS). <br />Read more about the CLS - https://web.dev/articles/cls', 'urlslab' ),
			self::OPTION_TYPE_CHECKBOX,
			false,
			null,
			'vitals'
		...
		$this->add_option_definition(
			...
			true,
			true,
			__( 'LCP Measurements', 'urlslab' ),
			__( 'Enable logging for Largest Contentful Paint (LCP). <br />Read more about the LCP - https://web.dev/articles/lcp', 'urlslab' ),
			self::OPTION_TYPE_CHECKBOX,
			false,
			null,
			'vitals'
		...
		$this->add_option_definition(
			...
			true,
			true,
			__( 'FCP Measurements', 'urlslab' ),
			__( 'Enable logging for First Contentful Paint (FCP). <br />Read more about the FCP - https://web.dev/articles/fcp', 'urlslab' ),
			self::OPTION_TYPE_CHECKBOX,
			false,
			null,
			'vitals'
		...
		$this->add_option_definition(
			...
			true,
			true,
			__( 'FID Measurements', 'urlslab' ),
			__( 'Enable logging for First Input Delay (FID). <br />Read more about the FID - https://web.dev/articles/fid', 'urlslab' ),
			self::OPTION_TYPE_CHECKBOX,
			false,
			null,
			'vitals'
		...
		$this->add_option_definition(
			...
			true,
			true,
			__( 'INP Measurements', 'urlslab' ),
			__( 'Enable logging for Interaction to Next Paint (INP). <br />Read more about the INP - https://web.dev/articles/inp', 'urlslab' ),
			self::OPTION_TYPE_CHECKBOX,
			false,
			null,
			'vitals'
		...
		$this->add_option_definition(
			...
			true,
			true,
			__( 'TTFB Measurements', 'urlslab' ),
			__( 'Enable logging for Time to First Byte (TTFB). <br />Read more about the TTFB - https://web.dev/articles/ttfb', 'urlslab' ),
			self::OPTION_TYPE_CHECKBOX,
			false,
			null,
			'vitals'
		...
		$this->add_option_definition(
			...
			0,
			true,
			__( 'Minimum Log Level', 'urlslab' ),
			__( 'Store only entries with a rating that is equal to or worse than the selected rating.', 'urlslab' ),
		...
		$this->add_option_definition(
			...
			'.*',
			true,
			__( 'URL Filter', 'urlslab' ),
			__( 'Measure the performance only for URLs that match the given regular expression. To include measurements for all pages on your website, enter the value `.*`.', 'urlslab' ),
		...
		$this->add_option_definition(
			...
			true,
			true,
			__( 'Attribution Measurements', 'urlslab' ),
			__( 'Measuring attributions can be complex and require more storage beyond basic logs. It is not advisable to extensively use this option in production. However, it is the only method to identify the underlying reason for subpar performance on specific pages. Once the main issue is pinpointed, we suggest disabling this option and solely monitoring performance metrics without logging in-depth data.', 'urlslab' ),
			self::OPTION_TYPE_CHECKBOX,
			false,
			null,
			'vitals'
		...
		if ( Urlslab_User_Widget::get_instance()->is_widget_activated( Urlslab_Widget_Optimize::SLUG ) ) {
			...
			$this->add_option_definition(
				...
				168,
				false,
				__( 'Time to keep web vitals logs (hours)', 'urlslab' ),
				__( 'Define time to keep the web vitals logs in your database (in hours). Default value: 168 hours = one week.', 'urlslab' ),
