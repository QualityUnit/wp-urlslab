...
class Urlslab_Widget_Urls extends Urlslab_Widget {
	...
	protected function add_options() {
		$this->add_options_form_section( 'main', __( 'Link Formatting and Monitoring', 'urlslab' ), __( 'This plugin seamlessly monitor link usage on your website during page view. Utilizing advanced data or preset enhancements, all links in the created HTML are evaluated and optimized for top performance.', 'urlslab' ), array( self::LABEL_FREE ) );

		$this->add_option_definition(
			self::SETTING_NAME_URLS_MAP,
			true,
			true,
			__( 'Monitor Link Usage', 'urlslab' ),
			__( 'It will automatically create and save a diagram illustrating the connections between your website\'s pages.', 'urlslab' ),
		...
		$this->add_option_definition(
			...
			true,
			__( 'Link Description', 'urlslab' ),
			__( 'The text that appears in the link\'s title/alt text. If the summary is not present, the meta description of the target URL is utilized.', 'urlslab' ),
		...
		$this->add_option_definition(
			...
			false,
			true,
			__( 'Improve Links Using Text Fragment', 'urlslab' ),
			__( 'Insert Text fragments into your website\'s links to enhance internal SEO and direct visitors to the relevant paragraph connected with the link. To bypass some links, add the `urlslab-skip-fragment` class name to either the link or sections housing these links.', 'urlslab' ),
			self::OPTION_TYPE_CHECKBOX,
			false,
			null,
			'main'
		...
		$this->add_option_definition(
			...
			true,
			true,
			__( 'Improve Links with Hreflang Attribute', 'urlslab' ),
			__( 'When the destination page of the link is analyzed and includes a lang attribute, improve each link with an hreflang attribute.', 'urlslab' ),
			self::OPTION_TYPE_CHECKBOX,
			false,
			null,
			'main'
		...
		$this->add_option_definition(
			...
			false,
			true,
			__( 'Insert Anchor IDs to Every Heading', 'urlslab' ),
			__( 'Improve all headers with ID attributes to enable users to link directly to a specific part of the website.', 'urlslab' ),
			self::OPTION_TYPE_CHECKBOX,
			false,
			null,
			'main'
		...
		$this->add_options_form_section( 'validation', __( 'Link Validation', 'urlslab' ), __( 'Maintaining your content\'s quality is a crucial SEO duty. Use the settings below to automate the dead or broken link checks on your website. This eliminates the manual hunt for these invalid links in your HTML content.', 'urlslab' ), array( self::LABEL_FREE ) );

		$this->add_option_definition(
			self::SETTING_NAME_VALIDATE_LINKS,
			true,
			false,
			__( 'Validate Links', 'urlslab' ),
			__( 'Verify the HTTP status of each link present in the website content.', 'urlslab' ),
			self::OPTION_TYPE_CHECKBOX,
			false,
			null,
			'validation'
		...
		$this->add_option_definition(
			...
			2419200,
			false,
			__( 'Validation Interval', 'urlslab' ),
			__( 'Define the frequency for URL status checks on your site. This feature may require significant processing resources. For optimal performance, we suggest monthly or quarterly check.', 'urlslab' ),
			self::OPTION_TYPE_LISTBOX,
			array(
		...
		$this->add_option_definition(
			...
			true,
			true,
			__( 'Mark URLs Processed by WordPress as Valid', 'urlslab' ),
			__( 'When WordPress delivers an unvalidated URL, we\'ll deem it valid.', 'urlslab' ),
			self::OPTION_TYPE_CHECKBOX,
			false,
			null,
			'validation'
		...
		$this->add_option_definition(
			...
			true,
			true,
			__( 'Hide Invalid Links or Links Marked as Hidden', 'urlslab' ),
			__( 'Hide all links in HTML content that are manually marked as hidden or point to invalid link (e.g. 404 Not Found, etc).', 'urlslab' ),
			self::OPTION_TYPE_CHECKBOX,
			false,
			null,
			'validation'
		...
		$this->add_option_definition(
			...
			true,
			true,
			__( 'Replace Redirected Links for Their Destination URLs', 'urlslab' ),
			__( 'Substitute all links that have redirects with their destination URLs.', 'urlslab' ),
			self::OPTION_TYPE_CHECKBOX,
			false,
			null,
			'validation'
		...
		$this->add_option_definition(
			...
			true,
			true,
			__( 'Replace Non-SEO Friendly Links', 'urlslab' ),
			__( 'Substitute all non-SEO friendly links with their SEO friendly versions, which search engines prefer. Currently, our support is limited to replacing `page_id` links. To omit certain links, assign the `urlslab-skip-page_id` class name to the link or to any elements that encompass the links.', 'urlslab' ),
			self::OPTION_TYPE_CHECKBOX,
			false,
			null,
			'validation'
		...
		$this->add_option_definition(
			...
			true,
			true,
			__( 'Hide Invalid Non-SEO Friendly Links', 'urlslab' ),
			__( 'Hide all links that have an incorrect `page_id` within the website content.', 'urlslab' ),
			self::OPTION_TYPE_CHECKBOX,
			false,
			null,
			'validation'
		...
		$this->add_option_definition(
			...
			false,
			true,
			__( 'Unify Protocol', 'urlslab' ),
			__( 'Ensure all links have the same protocol as the current domain.', 'urlslab' ),
			self::OPTION_TYPE_CHECKBOX,
			false,
			null,
			'validation'
		...
		$this->add_options_form_section( 'scheduling', __( 'Schedule Configuration', 'urlslab' ), __( 'Boost your page\'s content quality using AI-powered summarizations by URLsLab service for all your site\'s URLs. Enhance link titles and meta descriptions, giving visitors a clear preview of the content they\'ll find on the destination page.', 'urlslab' ), array( self::LABEL_PAID ) );
		$this->add_option_definition(
			self::SETTING_NAME_AUTMATICALLY_GENERATE_SUMMARY_INTERNAL_LINKS,
			false,
			true,
			__( 'Schedule All Internal Links', 'urlslab' ),
		...
		$this->add_option_definition(
			...
			false,
			true,
			__( 'Schedule All External Links', 'urlslab' ),
		...
		$this->add_options_form_section( 'meta', __( 'Meta Tags Configuration', 'urlslab' ), __( 'The plugin creates an amplified page summary serving as a description. It provides more detail than a conventional page description, aiding search engines to grasp your page\'s context, thus improving user findability in search results.', 'urlslab' ), array( self::LABEL_PAID ) );

		$this->add_option_definition(
			self::SETTING_NAME_META_DESCRIPTION_GENERATION,
			self::ADD_VALUE,
			true,
			__( 'Meta Description', 'urlslab' ),
			__( 'Add or replace the existing or absent meta description by summarizing the page\'s content.', 'urlslab' ),
			self::OPTION_TYPE_LISTBOX,
			array(
		...
		$this->add_option_definition(
			...
			self::ADD_VALUE,
			true,
			__( 'Title tag', 'urlslab' ),
			__( 'Add or replace the existing or absent title tag.', 'urlslab' ),
			self::OPTION_TYPE_LISTBOX,
			array(
				self::NO_CHANGE_VALUE => '-',
		...
		$this->add_options_form_section( 'og', __( 'Open Graph Meta Tags Configuration', 'urlslab' ), __( 'Open Graph meta tags bolster your content\'s visibility and sharability on social media platforms. They enhance your social media presence with interactive and engaging previews, attracting more clicks and attention.', 'urlslab' ) );

		$this->add_option_definition(
			self::SETTING_NAME_META_OG_TITLE_GENERATION,
			self::ADD_VALUE,
			true,
			__( 'Open Graph Title', 'urlslab' ),
			__( 'Add or replace the present or absent Open Graph title with the revised version.', 'urlslab' ),
			self::OPTION_TYPE_LISTBOX,
			array(
				self::NO_CHANGE_VALUE => '-',
				self::ADD_VALUE       => __( 'Add if missing', 'urlslab' ),
		...
		$this->add_option_definition(
			...
			self::ADD_VALUE,
			true,
			__( 'Open Graph Description', 'urlslab' ),
			__( 'Add or replace the existing or absent Open Graph description by summarizing the page\'s content.', 'urlslab' ),
			self::OPTION_TYPE_LISTBOX,
			array(
				self::NO_CHANGE_VALUE => '-',
				self::ADD_VALUE       => __( 'Add if missing', 'urlslab' ),
		...
		$this->add_option_definition(
			...
			self::ADD_VALUE,
			true,
			__( 'Open Graph Image', 'urlslab' ),
			__( 'Add or replace the existing or absent Open Graph image with a screenshot using the URLsLab service.', 'urlslab' ),
			self::OPTION_TYPE_LISTBOX,
			array(
		...
		$this->add_options_form_section( 'twitter', __( 'Twitter Card Meta Tags Configuration', 'urlslab' ), __( 'Enhance your content\'s visibility on social media using Twitter Card meta tags, providing engaging previews that attract users and boost shareability.', 'urlslab' ), array( self::LABEL_FREE ) );
		$this->add_option_definition(
			self::SETTING_NAME_TWITTER,
			self::ADD_VALUE,
			true,
			__( 'Twitter Card Meta Tags', 'urlslab' ),
			__( 'Add or replace the existing or absent Twitter Card description by summarizing the page\'s content.', 'urlslab' ),
			self::OPTION_TYPE_LISTBOX,
			array(
				self::NO_CHANGE_VALUE => '-',
				self::ADD_VALUE       => __( 'Add if missing', 'urlslab' ),
		...
		$this->add_option_definition(
			...
			true,
			__( 'Twitter Card Type', 'urlslab' ),
			__( 'Select a default style for the "twitter:card" meta field value.', 'urlslab' ),
			self::OPTION_TYPE_LISTBOX,
			array(
		...
		$this->add_option_definition(
			...
			'',
			true,
			__( 'Twitter Username', 'urlslab' ),
			__( 'Enter default Twitter username starting with the "@" symbol.', 'urlslab' ),
		...
		$this->add_option_definition(
			...
			'',
			true,
			__( 'Twitter Creator', 'urlslab' ),
			__( 'Enter default Twitter Creator username starting with the "@" symbol.', 'urlslab' ),
			self::OPTION_TYPE_TEXT,
			false,
			function( $value ) {
				return empty( $value ) || ( false !== strpos( $value, '@' ) && 1 < strlen( $value ) && 16 > strlen( $value ) );
		...
		$this->add_option_definition(
			...
			'',
			true,
			__( 'Twitter Player', 'urlslab' ),
			__( 'Enter player iframe URL (HTTPS only) for Player Card use. Leave blank if unsure.', 'urlslab' ),
			self::OPTION_TYPE_TEXT,
			false,
			function( $value ) {
		...
		$this->add_option_definition(
			...
			'',
			true,
			__( 'Twitter Player Width', 'urlslab' ),
			__( 'Enter Twitter Player iframe width in pixels.', 'urlslab' ),
			self::OPTION_TYPE_TEXT,
			false,
			function( $value ) {
		...
		$this->add_option_definition(
			...
			'',
			true,
			__( 'Twitter Player Height', 'urlslab' ),
			__( 'Enter Twitter Player iframe height in pixels.', 'urlslab' ),
			self::OPTION_TYPE_TEXT,
			false,
			function( $value ) {
				return empty( $value ) || is_numeric( $value );
		...
		$this->add_option_definition(
			...
			'',
			true,
			__( 'iPhone App Name', 'urlslab' ),
			__( 'Enter your iPhone app\'s name in the Apple App Store, applicable for App Card usage.', 'urlslab' ),
			self::OPTION_TYPE_TEXT,
			false,
			function( $value ) {
		...
		$this->add_option_definition(
			...
			'',
			true,
			__( 'iPhone App ID in the Apple App Store', 'urlslab' ),
			__( 'Enter your iPhone App ID in the Apple App Store, applicable for App Card usage.', 'urlslab' ),
			self::OPTION_TYPE_TEXT,
			false,
			function( $value ) {
				return empty( $value ) || is_string( $value );
		...
		$this->add_option_definition(
			...
			'',
			true,
			__( 'iPhone App\'s Custom URL Scheme', 'urlslab' ),
			__( 'Enter your iPhone app\'s custom URL Scheme, applicable for App Card usage.', 'urlslab' ),
			self::OPTION_TYPE_TEXT,
			false,
			function( $value ) {
				return empty( $value ) || is_string( $value );
		...
		$this->add_option_definition(
			...
			'',
			true,
			__( 'iPad App Name', 'urlslab' ),
			__( 'Enter your iPad app\'s name in the Apple App Store, applicable for App Card usage.', 'urlslab' ),
			self::OPTION_TYPE_TEXT,
			false,
			function( $value ) {
				return empty( $value ) || is_string( $value );
		...
		$this->add_option_definition(
			...
			'',
			true,
			__( 'iPad App ID in the Apple App Store', 'urlslab' ),
			__( 'Enter your iPad App ID in the Apple App Store, applicable for App Card usage.', 'urlslab' ),
			self::OPTION_TYPE_TEXT,
			false,
			function( $value ) {
				return empty( $value ) || is_string( $value );
		...
		$this->add_option_definition(
			...
			'',
			true,
			__( 'iPad App\'s Custom URL Scheme', 'urlslab' ),
			__( 'Enter your iPad app\'s custom URL Scheme, applicable for App Card usage.', 'urlslab' ),
			self::OPTION_TYPE_TEXT,
			false,
			function( $value ) {
				return empty( $value ) || is_string( $value );
		...
		$this->add_option_definition(
			...
			'',
			true,
			__( 'Android App Name', 'urlslab' ),
			__( 'Enter your Android app\'s name in the Google Play Store, applicable for App Card usage.', 'urlslab' ),
			self::OPTION_TYPE_TEXT,
			false,
			function( $value ) {
				return empty( $value ) || is_string( $value );
		...
		$this->add_option_definition(
			...
			'',
			true,
			__( 'App ID in the Google Play Store', 'urlslab' ),
			__( 'Enter your Android App ID in the Google Play Store, applicable for App Card usage.', 'urlslab' ),
			self::OPTION_TYPE_TEXT,
			false,
			function( $value ) {
				return empty( $value ) || is_string( $value );
		...
		$this->add_option_definition(
			...
			'',
			true,
			__( 'Android App\'s Custom URL Scheme', 'urlslab' ),
			__( 'Enter your Android app\'s custom URL Scheme, applicable for App Card usage.', 'urlslab' ),
			self::OPTION_TYPE_TEXT,
			false,
			function( $value ) {
				return empty( $value ) || is_string( $value );
		...
		$this->add_options_form_section( 'image', __( 'Image Alt Text Attribute', 'urlslab' ), __( 'Alt text is essential for accessibility, offering a text substitute for individuals with visual impairments. It also assists search engines in comprehending the image\'s content, promoting appropriate indexing and keyword association.', 'urlslab' ), array( self::LABEL_FREE ) );
		$this->add_option_definition(
			self::SETTING_NAME_ALT_TAG_SOURCE,
			array( self::SOURCE_FIGCAPTION, self::SOURCE_LINK, self::SOURCE_H ),
			true,
			__( 'Alt Text Source', 'urlslab' ),
			__( 'Select the Alt attribute text source providing a fallback function. The system will automatically switch to a fallback choice if the selected source is unavailable.', 'urlslab' ),
