...
class Urlslab_Widget_Security extends Urlslab_Widget {
	...
	public function get_widget_title(): string {
		return __( 'Security', 'wp-urlslab' );
	}

	public function get_widget_description(): string {
		return __( 'Protect your page against some basic attacks on your WordPress installation.', 'wp-urlslab' );
	}
	...
	protected function add_options() {
		...
		$this->add_options_form_section(
			'save',
			function () {
				return __( 'Security settings', 'wp-urlslab' );
			},
			function () {
				return __( 'Most of the security settings are applied to your website through `.htaccess` file rules. To apply your current security settings you need to apply it by updating `.htaccess` file', 'wp-urlslab' );
			},
			array(
				self::LABEL_FREE,
			)
		...
		$this->add_options_form_section(
			'sec-headers',
			function () {
				return __( 'Security headers', 'wp-urlslab' );
			},
			function () {
				return __( 'Protect your installation against basic attacks with standard security settings of HTTP headers.', 'wp-urlslab' );
			},
			array(
				self::LABEL_FREE,
			)
		...
		$this->add_option_definition(
			...
			true,
			function () {
				return __( 'Add Referrer-Policy header', 'wp-urlslab' );
			},
			function () {
				return __( 'The Referrer-Policy HTTP header controls how much referrer information (sent with the Referer header) should be included with requests. Read more: https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Referrer-Policy', 'wp-urlslab' );
			},
			self::OPTION_TYPE_LISTBOX,
			function () {
				return array(
					'none'                            => __( 'Not used', 'wp-urlslab' ),
					'no-referrer'                     => __( 'no-referrer', 'wp-urlslab' ),
					'no-referrer-when-downgrade'      => __( 'no-referrer-when-downgrade', 'wp-urlslab' ),
					'origin'                          => __( 'origin', 'wp-urlslab' ),
					'origin-when-cross-origin'        => __( 'origin-when-cross-origin', 'wp-urlslab' ),
					'same-origin'                     => __( 'same-origin', 'wp-urlslab' ),
					'strict-origin'                   => __( 'strict-origin', 'wp-urlslab' ),
					'strict-origin-when-cross-origin' => __( 'strict-origin-when-cross-origin', 'wp-urlslab' ),
					'unsafe-url'                      => __( 'unsafe-url', 'wp-urlslab' ),
				);
			},
			null,
			'sec-headers',
		...
		$this->add_option_definition(
			...
			true,
			function () {
				return __( 'Add X-Frame-Options header', 'wp-urlslab' );
			},
			function () {
				return __( 'The X-Frame-Options HTTP response header can be used to indicate whether or not a browser should be allowed to render a page in a `frame`, `iframe`, `embed` or `object`. Sites can use this to avoid click-jacking attacks, by ensuring that their content is not embedded into other sites. Read more: https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/X-Frame-Options', 'wp-urlslab' );
			},
			self::OPTION_TYPE_LISTBOX,
			function () {
				return array(
					'none'       => __( 'Not used', 'wp-urlslab' ),
					'DENY'       => __( 'DENY - The page cannot be displayed in a frame', 'wp-urlslab' ),
					'SAMEORIGIN' => __( 'SAMEORIGIN - Allow if all ancestor frames are same origin', 'wp-urlslab' ),
				);
			},
			null,
			'sec-headers',
		...
		$this->add_option_definition(
			...
			false,
			function () {
				return __( 'Add Permissions-Policy header', 'wp-urlslab' );
			},
			function () {
				return __( 'The HTTP Permissions-Policy header provides a mechanism to allow and deny the use of browser features in a document or within any `iframe` elements in the document. Read more: https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Permissions-Policy', 'wp-urlslab' );
			},
		...
		$this->add_option_definition(
			...
			false,
			function () {
				return __( 'Add Strict-Transport-Security header', 'wp-urlslab' );
			},
			function () {
		...
		$this->add_option_definition(
			...
			function () {
				return __( 'Activate rate-limits of page views per IP address of visitor.', 'wp-urlslab' );
			},
			self::OPTION_TYPE_CHECKBOX,
			false,
			null,
		...
		$this->add_option_definition(
			...
			true,
			function () {
				return __( 'Rate limit page views from same IP (per minute)', 'wp-urlslab' );
			},
			function () {
				return __( 'Implement a rate limit to restrict the number of page views per minute from a single IP address, to prevent server overload. This security measure helps to deter automated traffic and potential abuse, ensuring fair resource distribution. Users exceeding the set page view limit will temporarily be blocked, maintaining site accessibility for others.', 'wp-urlslab' );
			},
		...
		$this->add_options_form_section(
			'404',
			function () {
				return __( 'Rate-limit 404 Not Found Page', 'wp-urlslab' );
			},
			function () {
		...
		$this->add_option_definition(
			...
			true,
			function () {
				return __( 'Rate limit 404', 'wp-urlslab' );
			},
			function () {
				return __( 'Activate rate-limits of page views to 404 Not Found pages per IP address of visitor.', 'wp-urlslab' );
			},
			self::OPTION_TYPE_CHECKBOX,
			false,
			null,
		...
		$this->add_option_definition(
			...
			true,
			function () {
				return __( 'Rate limit 404 attempts from IP (per minute)', 'wp-urlslab' );
			},
			function () {
				return __( 'If visitor from specific address executes more requests per minute to not existing urls, he will be blocked for defined amount of time. Lower the number you set, more strict is the protection. If your page contains e.g. many links to not existing urls, it can lock out even real visitor.', 'wp-urlslab' );
			},
			self::OPTION_TYPE_NUMBER,
			false,
			null,
		...
		$this->add_option_definition(
			...
			true,
			function () {
				return __( 'IP blocking time', 'wp-urlslab' );
			},
			function () {
				return __( 'Enter time in seconds to block IP address attacking your website through not found pages. If set to 0, no IP will be blocked.', 'wp-urlslab' );
			},
			self::OPTION_TYPE_NUMBER,
			false,
			null,
		...
		$this->add_option_definition(
			...
			true,
			function () {
				return __( 'Add CSP headers', 'wp-urlslab' );
			},
			function () {
				return __( 'Add to each response from your server CSP header to protect your server. Activate it just in case you know what you are doing. IMPORTANT: Use `Report only` option just for limited amount of time during debugging. Reporting can significantly slow down your server!', 'wp-urlslab' );
			},
			self::OPTION_TYPE_LISTBOX,
			function () {
				return array(
					'none'    => __( 'No CSP headers', 'wp-urlslab' ),
					'report'  => __( 'Report only', 'wp-urlslab' ),
					'enforce' => __( 'Enforce CSP headers', 'wp-urlslab' ),
				);
			},
			null,
			'csp',
		...
		$this->add_option_definition(
			...
			true,
			function () {
				return __( 'default-src', 'wp-urlslab' );
			},
			function () {
				return __( 'Serves as a fallback for the other fetch directives. Read more: https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/default-src', 'wp-urlslab' );
			},
			self::OPTION_TYPE_TEXT,
			false,
			function ( $value ) {
		...
		$this->add_option_definition(
			...
			true,
			function () {
				return __( 'child-src', 'wp-urlslab' );
			},
			function () {
				return __( 'Defines the valid sources for web workers and nested browsing contexts loaded using elements such as `frame` and `iframe`. Read more: https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/child-src', 'wp-urlslab' );
			},
			self::OPTION_TYPE_TEXT,
			false,
			function ( $value ) {
		...
		$this->add_option_definition(
			...
			true,
			function () {
				return __( 'base-uri', 'wp-urlslab' );
			},
			function () {
				return __( "The HTTP Content-Security-Policy base-uri directive restricts the URLs which can be used in a document's `base` element. If this value is absent, then any URI is allowed. If this directive is absent, the user agent will use the value in the `base` element. Read more: https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/base-uri", 'wp-urlslab' );
			},
			self::OPTION_TYPE_TEXT,
			false,
			function ( $value ) {
		...
		$this->add_option_definition(
			...
			true,
			function () {
				return __( 'connect-src', 'wp-urlslab' );
			},
			function () {
		...
		$this->add_option_definition(
			...
			true,
			function () {
				return __( 'font-src', 'wp-urlslab' );
			},
			function () {
				return __( 'Specifies valid sources for fonts loaded using `@font-face`. Read more: https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/font-src', 'wp-urlslab' );
			},
			self::OPTION_TYPE_TEXT,
			false,
			function ( $value ) {
		...
		$this->add_option_definition(
			...
			true,
			function () {
				return __( 'frame-src', 'wp-urlslab' );
			},
			function () {
				return __( 'Specifies valid sources for nested browsing contexts loading using elements such as `frame` and `iframe`. Read more: https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/frame-src', 'wp-urlslab' );
			},
			self::OPTION_TYPE_TEXT,
			false,
			function ( $value ) {
		...
		$this->add_option_definition(
			...
			true,
			function () {
				return __( 'img-src', 'wp-urlslab' );
			},
			function () {
				return __( 'Specifies valid sources of images and favicons. Read more: https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/img-src', 'wp-urlslab' );
			},
			self::OPTION_TYPE_TEXT,
			false,
			function ( $value ) {
		...
		$this->add_option_definition(
			...
			true,
			function () {
				return __( 'manifest-src', 'wp-urlslab' );
			},
			function () {
				return __( 'Specifies valid sources of application manifest files. Read more: https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/manifest-src', 'wp-urlslab' );
			},
			self::OPTION_TYPE_TEXT,
			false,
			function ( $value ) {
		...
		$this->add_option_definition(
			...
			true,
			function () {
				return __( 'media-src', 'wp-urlslab' );
			},
			function () {
				return __( 'Specifies valid sources for loading media using the `audio` , `video` and `track` elements. Read more: https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/media-src', 'wp-urlslab' );
			},
			self::OPTION_TYPE_TEXT,
			false,
			function ( $value ) {
		...
		$this->add_option_definition(
			...
			true,
			function () {
				return __( 'object-src', 'wp-urlslab' );
			},
			function () {
				return __( 'The HTTP Content-Security-Policy object-src directive specifies valid sources for the `object` and `embed` elements. Read more: https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/object-src', 'wp-urlslab' );
			},
			self::OPTION_TYPE_TEXT,
			false,
			function ( $value ) {
		...
		$this->add_option_definition(
			...
			true,
			function () {
				return __( 'script-src', 'wp-urlslab' );
			},
			function () {
				return __( 'Specifies valid sources for JavaScript and WebAssembly resources. Read more: https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/script-src', 'wp-urlslab' );
			},
			self::OPTION_TYPE_TEXT,
			false,
			function ( $value ) {
		...
		$this->add_option_definition(
			...
			function () {
				return array(
					'allow-downloads'                         => __( 'allow-downloads', 'wp-urlslab' ),
					'allow-downloads-without-user-activation' => __( 'allow-downloads-without-user-activation', 'wp-urlslab' ),
					'allow-forms'                             => __( 'allow-forms', 'wp-urlslab' ),
					'allow-modals'                            => __( 'allow-modals', 'wp-urlslab' ),
					'allow-orientation-lock'                  => __( 'allow-orientation-lock', 'wp-urlslab' ),
					'allow-pointer-lock'                      => __( 'allow-pointer-lock', 'wp-urlslab' ),
					'allow-popups'                            => __( 'allow-popups', 'wp-urlslab' ),
					'allow-popups-to-escape-sandbox'          => __( 'allow-popups-to-escape-sandbox', 'wp-urlslab' ),
					'allow-presentation'                      => __( 'allow-presentation', 'wp-urlslab' ),
					'allow-same-origin'                       => __( 'allow-same-origin', 'wp-urlslab' ),
					'allow-scripts'                           => __( 'allow-scripts', 'wp-urlslab' ),
					'allow-storage-access-by-user-activation' => __( 'allow-storage-access-by-user-activation', 'wp-urlslab' ),
					'allow-top-navigation'                    => __( 'allow-top-navigation', 'wp-urlslab' ),
					'allow-top-navigation-by-user-activation' => __( 'allow-top-navigation-by-user-activation', 'wp-urlslab' ),
				);
			},
			null,
			'csp'
		...
		$this->add_option_definition(
			...
			true,
			function () {
				return __( 'script-src-elem', 'wp-urlslab' );
			},
			function () {
				return __( 'Specifies valid sources for JavaScript `script` elements. Read more: https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/script-src-elem', 'wp-urlslab' );
			},
			self::OPTION_TYPE_TEXT,
			false,
			function ( $value ) {
		...
		$this->add_option_definition(
			...
			true,
			function () {
				return __( 'script-src-attr', 'wp-urlslab' );
			},
			function () {
				return __( 'Specifies valid sources for JavaScript inline event handlers. Read more: https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/script-src-attr', 'wp-urlslab' );
			},
			self::OPTION_TYPE_TEXT,
			false,
			function ( $value ) {
		...
		$this->add_option_definition(
			...
			true,
			function () {
				return __( 'style-src', 'wp-urlslab' );
			},
			function () {
				return __( 'Specifies valid sources for stylesheets. Read more: https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/style-src', 'wp-urlslab' );
			},
			self::OPTION_TYPE_TEXT,
			false,
			function ( $value ) {
		...
		$this->add_option_definition(
			...
			true,
			function () {
				return __( 'style-src-elem', 'wp-urlslab' );
			},
			function () {
				return __( 'Specifies valid sources for stylesheets `style` elements and `link` elements with `rel="stylesheet"`. Read more: https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/style-src-elem', 'wp-urlslab' );
			},
			self::OPTION_TYPE_TEXT,
			false,
			function ( $value ) {
		...
		$this->add_option_definition(
			...
			true,
			function () {
				return __( 'style-src-attr', 'wp-urlslab' );
			},
			function () {
				return __( 'Specifies valid sources for inline styles applied to individual DOM elements.', 'wp-urlslab' );
			},
			self::OPTION_TYPE_TEXT,
			false,
			function ( $value ) {
		...
		$this->add_option_definition(
			...
			true,
			function () {
				return __( 'worker-src', 'wp-urlslab' );
			},
			function () {
				return __( 'Specifies valid sources for Worker, SharedWorker, or ServiceWorker scripts. Read more: https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/worker-src', 'wp-urlslab' );
			},
			self::OPTION_TYPE_TEXT,
			false,
			function ( $value ) {
		...
		$this->add_option_definition(
			...
			true,
			function () {
				return __( 'form-action', 'wp-urlslab' );
			},
			function () {
				return __( 'Restricts the URLs which can be used as the target of a form submissions from a given context. Read more: https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/form-action', 'wp-urlslab' );
			},
			self::OPTION_TYPE_TEXT,
			false,
			function ( $value ) {
		...
		$this->add_option_definition(
			...
			true,
			function () {
				return __( 'upgrade-insecure-requests', 'wp-urlslab' );
			},
			function () {
				return __( "The HTTP Content-Security-Policy (CSP) upgrade-insecure-requests directive instructs user agents to treat all of a site's insecure URLs (those served over HTTP) as though they have been replaced with secure URLs (those served over HTTPS). This directive is intended for websites with large numbers of insecure legacy URLs that need to be rewritten. Read more: https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Security-Policy/upgrade-insecure-requests", 'wp-urlslab' );
			},
			self::OPTION_TYPE_CHECKBOX,
			false,
			null,
		...
		$this->add_option_definition(
			...
			true,
			function () {
				return __( 'Violations Tracking', 'wp-urlslab' );
			},
			function () {
				return __( 'Choose how will be tracked the violation. Javascript version will aggregate multiple violations into single http request, but can fail to be executed. Browser tracking will be independent on javascript, but could trigger huge number of requests to your server and overpower it.', 'wp-urlslab' );
			},
			self::OPTION_TYPE_LISTBOX,
			function () {
				return array(
					self::TRACK_WITH_JAVASCRIPT => __( 'Aggregated with JS', 'wp-urlslab' ),
					'browser'                   => __( 'Browser - one request per violation', 'wp-urlslab' ),
				);
			},
			null,
			'csp'
