...
class Urlslab_Widget_Related_Resources extends Urlslab_Widget {
	...
	private function load_related_urls( string $url_id, int $limit ): array {
		...
		return $wpdb->get_results( $wpdb->prepare( $q, $url_id, $url_id, Urlslab_Data_Url::VISIBILITY_VISIBLE, Urlslab_Data_Url::HTTP_STATUS_OK, Urlslab_Data_Url::HTTP_STATUS_NOT_PROCESSED, Urlslab_Data_Url::HTTP_STATUS_PENDING, $limit ), ARRAY_A ); // phpcs:ignore
	}
	...
	protected function add_options() {
		...
		$this->add_option_definition(
			...
			self::OPTION_TYPE_LISTBOX,
			array(
				Urlslab_Data_Url::SCREENSHOT_TYPE_CAROUSEL_THUMBNAIL  => __( 'Top Part Thumbnail (200px x 112px)' ),
				Urlslab_Data_Url::SCREENSHOT_TYPE_CAROUSEL            => __( 'Top Part (1358px x 642px)' ),
				Urlslab_Data_Url::SCREENSHOT_TYPE_FULL_PAGE_THUMBNAIL => __( 'Full Page Thumbnail (200px x dynamic height)' ),
				Urlslab_Data_Url::SCREENSHOT_TYPE_FULL_PAGE           => __( 'Full Page (1358px x dynamic height)' ),
			),
			function( $value ) {
				switch ( $value ) {
