	...
	public function get_shortcode_content( $atts = array(), $content = null, $tag = '' ): string {
		...
		try {
			...
			if ( empty( $result ) && Urlslab_User_Widget::get_instance()->get_widget( Urlslab_Widget_General::SLUG )->get_option( Urlslab_Widget_General::SETTING_NAME_DEBUG ) ) {
				$content .= '<!-- No related articles found for url id: ' . esc_html( $shortcode_url->get_url_id() ) . ', url: ' . esc_html( $shortcode_url->get_url() ) . ', (Current page: ' . esc_html( Urlslab_Url::get_current_page_url()->get_url() ) . ')-->';
			}
	...
	private function render_shortcode_item( Urlslab_Data_Url $url_obj, array $urlslab_atts, $strategy ): string {
		try {
			...
			return '<div class="urlslab-rel-res-item urlslab-skip-keywords">' .
				   ...
				   ( $url_obj->get_url()->is_wp_domain() ? '' : ' target="_blank"' ) .
				   '>' .
				   esc_html( $title ) .
				   '</a>' .
				   '</p>' .
