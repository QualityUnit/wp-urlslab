...
class Urlslab_Widget_General extends Urlslab_Widget {
	...
	protected function add_options() {
		...
		$this->add_options_form_section( 'summarization', __( 'Summarizations' ), __( 'Summarizations are generated by the URLsLab service. Adjust the synchronization frequency between URLsLab service and the plugin here.' ), array( self::LABEL_PAID ) );
		$this->add_option_definition(
			self::SETTING_NAME_SUMMARIZATION_REFRESH_INTERVAL,
			2419200,
			false,
			__( 'Synchronization with URLsLab Service' ),
		...
		$this->add_options_form_section( 'screenshots', __( 'Screenshots' ), __( 'The URLsLab Screenshot service, is a premium feature of the plugin - generates screenshot of any URL.' ), array( self::LABEL_PAID ) );
		$this->add_option_definition(
			self::SETTING_NAME_SHEDULE_SCRRENSHOT,
			self::SCHEDULE_SHORTCODE,
			false,
			__( 'URL types' ),
			__( 'Select the types of URLs for which should be generated screenshot.' ),
			self::OPTION_TYPE_LISTBOX,
			array(
				self::SCHEDULE_NEVER         => __( 'None' ),
		...
		$this->add_option_definition(
			...
			false,
			__( 'Screenshot Synchronization' ),
			__( 'Choose the frequency at which plugin synchronize screenshots with URLsLab service. ' ),
			self::OPTION_TYPE_LISTBOX,
			array(
		...
		$this->add_options_form_section( 'disallowed', __( 'Blacklisted Domains' ), __( 'Preserve your server\'s computational capacity and cut down on costs associated with operations such as screen captures or summaries on unrelated domains or URLs to your SEO strategy.' ) );
		$this->add_option_definition(
