...
class Urlslab_Api_Keywords extends Urlslab_Api_Table {
	...
	public function get_kw_mapping_count( $request ) {
		return new WP_REST_Response(
			$this->get_kw_mapping_sql( $request )->get_count(), 200
		);
	}
	...
	public function delete_all_items( $request ) {
		...
		if ( false === $wpdb->query(
				$wpdb->prepare( 'TRUNCATE ' . URLSLAB_KEYWORDS_TABLE )
			)
		) { // phpcs:ignore
			return new WP_Error(
				'error', __( 'Failed to delete', 'urlslab' ),
		...
		if ( false === $wpdb->query(
				$wpdb->prepare( 'TRUNCATE ' . URLSLAB_KEYWORDS_MAP_TABLE )
			)
		) { // phpcs:ignore
			return new WP_Error(
				'error', __( 'Failed to delete', 'urlslab' ),
	...
	public function get_kw_mapping_sql( $request ): Urlslab_Api_Table_Sql {
		...
		$sql->add_from(
			URLSLAB_KEYWORDS_MAP_TABLE . ' m LEFT JOIN ' . URLSLAB_URLS_TABLE
