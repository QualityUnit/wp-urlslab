...
class Urlslab_Related_Resources_Widget extends Urlslab_Widget {
	...
	private function load_related_urls( string $url_id, int $limit ): array {
		...
		$q                  = "SELECT DISTINCT u.url_name FROM $related_urls_table r INNER JOIN $urls_table as u ON r.dest_url_id = u.url_id WHERE r.src_url_id = %d AND r.dest_url_id <> %d AND u.visibility = '%s' AND u.http_status IN (%d, %d, %d) ORDER BY r.pos LIMIT %d";
