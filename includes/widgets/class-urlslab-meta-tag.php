...
class Urlslab_Meta_Tag extends Urlslab_Widget {
	...
	public function get_widget_description(): string {
		return __( 'Boost your content\'s reach by implementing Meta Tags, increasing its shareability on all social media' );
	}
	...
	public function content_hook( DOMDocument $document ) {
		...
		try {
			...
			if ( is_object( $url_data ) && $url_data->is_http_valid() ) {
				...
				if ( strlen( $url_data->get_screenshot_url( Urlslab_Url_Row::SCREENSHOT_TYPE_CAROUSEL, true ) ) && $this->set_meta_tag( $document, $head_tag, 'meta', 'property', 'og:image', self::SETTING_NAME_META_OG_IMAGE_GENERATION, $url_data->get_screenshot_url() ) ) {
