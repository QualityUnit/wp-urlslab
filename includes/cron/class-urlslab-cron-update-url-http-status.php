...
class Urlslab_Cron_Update_Url_Http_Status extends Urlslab_Cron {
	...
	protected function execute(): bool {
		...
		if ( ! $widget->get_option( Urlslab_Widget_Link_Enhancer::SETTING_NAME_VALIDATE_LINKS ) ) {
			return false;
		}
	...
	private function updateUrl( Urlslab_Data_Url $url ) {
		try {
			...
			if ( Urlslab_Data_Url::HTTP_STATUS_OK <= $status_obj->code && 400 > $status_obj->code && $final_url->get_url_id() == $url->get_url_id() && $this->is_html_extension( $url->get_url()->get_extension() ) ) {
					...
					if ( empty( $page_content_file_name ) || ! file_exists( $page_content_file_name ) || 0 == filesize( $page_content_file_name ) ) {
						...
						try {
							...
							if ( $fp ) {
								...
								if ( strlen( $html_tag ) ) {
									if ( preg_match( '/<html[^>]+lang=([^\s>]+)/i', $html_tag, $match ) ) {
										...
										$url->set_url_lang( Urlslab_Data_Url::VALUE_EMPTY );
									}
								}
							}
							...
							if ( strlen( $html_text ) ) {
								...
								if ( empty( $url->get_url_lang() ) || Urlslab_Data_Url::VALUE_EMPTY == $url->get_url_lang() ) {
									$xpath     = new DOMXPath( $document );
			...
			if ( 300 < $status_obj->code && 399 > $status_obj->code && $final_url->get_url_id() == $url->get_url_id() ) {
				$url->set_http_status( Urlslab_Data_Url::HTTP_STATUS_OK );    //if the url is same as original, then it is OK and not redirect
