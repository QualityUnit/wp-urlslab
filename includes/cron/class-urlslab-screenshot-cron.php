...
function handle_schedules( $schedules ) {
	...
	if ( ! empty( $rsp ) ) {
		foreach ( $schedules as $i => $schedule ) {
			save_new_schedule_status( $rsp[ $i ], md5( $schedule['urlName'] ) );
		}
	}
}

function save_new_schedule_status( $urlslab_screenshot, $url_md5_hash ) {
	global $wpdb;
