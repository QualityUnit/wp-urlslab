...
URLSLAB_PATH="$GITHUB_WORKSPACE/urlslab"
...
svn co https://plugins.svn.wordpress.org/urlslab/trunk
...
echo "Copy files"
rsync -ah --progress $URLSLAB_PATH/* $SVN_PATH/trunk
...
svn cp https://plugins.svn.wordpress.org/urlslab/trunk https://plugins.svn.wordpress.org/urlslab/tags/${PLUGIN_VERSION} --message "Tagged ${PLUGIN_VERSION}" --no-auth-cache --non-interactive  --username "$SVN_USERNAME" --password "$SVN_PASSWORD"
svn update
